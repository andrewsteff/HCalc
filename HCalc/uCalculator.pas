...
begin
  ...
  begin
    ...
    FDisplay.UpdateUI(FOperNum, FOperand[intHistoryOperand],
...
var
  HistoryAction: THistoryUpdateType;
begin
...
begin
  HistoryAction := huptReplaceLast;
  if FOperandClosed then
    FOperand[FOperNum] := '0';

  if (Pos(FDecSep, FOperand[FOperNum]) = 0) then
  begin
    if OperandIsEmpty(FOperand[FOperNum]) then
      HistoryAction := huptAddNew;
...
begin
  ...
  begin
    ...
    begin
      OpResult := FSign[1] * StrToFloatDef(FOperand[1], 0) + FSign[2] *
        StrToFloatDef(FOperand[2], 0);
    end
    ...
    begin
      OpResult := (FSign[1] * StrToFloatDef(FOperand[1], 0)) /
        (FSign[2] * StrToFloatDef(FOperand[2], 0));
    end;
    ...
    FOperNum := 1;
    FOperandClosed := true;
    // current operand (first operand) is closed, any digit or separator will overwrite it
    FHistory.UpdateHistory(FOperand[1], huptAddNew);
    FDisplay.UpdateUI(FOperNum, FOperand[1], FHistory.HistoryToString, FMemory);
  end;
end;
...
begin
  result := false;
  if (strOperand = '0') or (strOperand = '') or (FOperandClosed) then
    result := true;
end;

procedure TCalculator.CalcMemory(mdir: integer);
var
  memOp: integer; // memOp = 0 - standart operation with memory
  // memOp = 1 - Operand_2 was empty, put Memory into Op_2 and dont change memory
begin
  memOp := 0;
  if (FOperNum = 2) then
  begin
